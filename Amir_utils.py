...
def extract_grayscale_patches( img, shape, offset=(0,0), stride=(1,1) ):
    ...
    x_tmp =  numpy.arange(offset[1],img.shape[1]-shape[1]+1,stride[1])
    y_tmp =  numpy.arange(offset[0], img.shape[0] - shape[0] + 1, stride[0])
    if img.shape[1]%shape[1]!=0:
        x_offset = img.shape[1]-shape[1]
        x_tmp = numpy.append(x_tmp,x_offset)
    if img.shape[0]%shape[0]!=0:
        y_offset = img.shape[0]-shape[0]
        y_tmp = numpy.append(y_tmp,y_offset)
